"""
This file mostly works at this point in time.
DATE: 12/19/08
PROBLEMS:
   #I canot pinpoint the variable(s) in the Field class which offset the
background on the screen without being unable to reach the unscreened parts
offsetted

   #I have yet to detail the code so the Player moves when the background
is at boundaries

   #Code is starting to get messy. Maybe a revamp with an object that handles
file information so I don't have excessive dummy finctions to get certain
variables...
"""
import os, sys
import pygame
...
class AI_Modes(object):
    def __init__(self):
        self.modes = dict()
        self.current_mode = None

    def add_mode(self, state):
        pass
######################################################
...
def get_spikes(filename = "001.ens"):
    slist = []
    fp = open(filename)
    for line in fp:
